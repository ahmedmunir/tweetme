...
const edit_event_listener = (tweet, e_event_handler) => {
    ...
    cancel_edit_button.classList.add('btn-danger');
...
const fetchData = () => {
    ...
    let username = document.querySelector('.user_username').dataset.username;
    let url = `${window.location.protocol}//${window.location.hostname}:${window.location.port}/${username}/tweets/?${params}`;
    fetch(url, {
        method: "GET",
        headers: {
